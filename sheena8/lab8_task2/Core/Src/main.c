...
static uint8_t tx_cmd = (uint8_t)(0x80 | (GYRO_OUT_TEMP & 0x3F)); // READ single byte (no auto-inc)
static uint8_t rx_byte = 0;

static float T0_C = 20.0f;   
static int8_t raw0 = 0;      
/* USER CODE END PV */
...
// SPI Tx complete: now receive 1 data byte
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
  if (hspi->Instance == SPI1)
  {
    HAL_SPI_Receive_IT(&hspi1, &rx_byte, 1);
  }
}

// SPI Rx complete: finish transaction, convert to degC, print ASCII
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi)
{
  if (hspi->Instance == SPI1)
  {
    ...
    int8_t raw = (int8_t)rx_byte;
    // L3GD20 temp sensitivity ≈ -1 LSB/°C; apply 1-point offset
    // T(°C) ≈ T0_C - (raw - raw0)
    float T_C = T0_C - ((float)raw - (float)raw0);

    char line[32];
    // Round to nearest int for clean output (prints like 18, 19, 20)
    snprintf(line, sizeof(line), "T=%d\r\n", (int)(T_C + (T_C >= 0 ? 0.5f : -0.5f)));
    uart_print(line);
  }
}
