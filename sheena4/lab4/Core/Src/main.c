...
void delay_ms(uint32_t ms)
{
    ...
    HAL_TIM_Base_Start(&htim2);        // start timer (base mode)

    while (__HAL_TIM_GET_COUNTER(&htim2) < ms) {
        // busy-wait (polling) — 1 tick = 1 ms now
    }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
        HAL_GPIO_TogglePin(GPIOE, LD10_Pin);  // any LED pin you like (LD3..LD10 on Discovery)
    }
}
