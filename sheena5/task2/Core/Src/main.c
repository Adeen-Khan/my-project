...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body (Task 2 - Motor Speed Control)
  ******************************************************************************
  */
...
void Motor_SetSpeed(uint16_t speed)
{
    if (speed > 999) speed = 999; //clamp to ARR
...
int main(void)
{
  ...
  Motor_SetSpeed(500);  //50% duty cycle
...
static void MX_GPIO_Init(void)
{
  ...
  __HAL_RCC_GPIOD_CLK_ENABLE();  // Enable clock for port D (for PD6, PD7)
  ...
  /* Note: No more PA1 and PA2 outputs here since direction pins moved to PD6 and PD7 */

  /* Other GPIO configurations (if any) remain unchanged */
